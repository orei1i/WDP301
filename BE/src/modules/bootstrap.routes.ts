...
import {
  AuditLogModel, FacilityModel, InspectionModel, PaymentModel, PolicyModel, RentalContractModel, ReservationModel,
  StorageUnitModel, TicketModel, UnitTypeModel, UserModel,
} from '../db/models';
import { authenticate } from '../middlewares/authenticate';
import { isScoped } from '../middlewares/scope';
...
bootstrapRouter.get('/', authenticate, async (req, res) => {
  ...
  const empty = { users: [], facilities: [], unitTypes: [], units: [], reservations: [], contracts: [], payments: [], inspections: [], tickets: [], policies: [], audit: [] };

  if (me.role === 'CUSTOMER') {
    const [facilities, unitTypes, reservations, contracts, payments, rawTickets] = await Promise.all([
    ...
    return res.json({ ...empty, me, users: [me, ...others], facilities, unitTypes, units, reservations, contracts, payments, inspections, tickets, policies });
  }
  ...
  const [facilities, unitTypes, units, reservations, contracts, payments, inspections, tickets, policies, audit] = await Promise.all([
  ...
  res.json({ me, users, facilities, unitTypes, units, reservations, contracts, payments, inspections, tickets, policies, audit });
});
